  ...
  return (
    <div className="min-h-screen bg-gray-100 p-2 sm:p-4">
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div className="flex items-center space-x-2 p-3 sm:p-4 border-b bg-gray-50">
          <ArrowLeft
            className="w-4 h-4 sm:w-5 sm:h-5 cursor-pointer"
            onClick={() => navigate(-1)}
          />
          <h2 className="text-lg sm:text-xl font-semibold">POST YOUR AD</h2>
        </div>


        <div className="bg-gray-200 p-2 sm:p-3 text-center font-semibold text-gray-600 text-sm sm:text-base">
          CHOOSE A CATEGORY
        </div>
        ...
        <div className="flex flex-col sm:grid sm:grid-cols-3 sm:divide-x">
            ...
            <ul className="space-y-1 sm:space-y-2">
              {categories.map((category, index) => (
                <li
                  ...
                  className={`flex items-center justify-between p-2 sm:p-3 rounded-lg cursor-pointer 
          ...
          <div className="col-span-2 p-3 sm:p-4">
            {selectedCategory ? (
              <div>
                ...
                <ul className="space-y-1 sm:space-y-2">
                  {selectedCategory.subcategories.map((sub, idx) => (
                    <li
                      ...
                      className="p-2 sm:p-2 rounded-lg bg-gray-100 hover:bg-gray-200 cursor-pointer text-sm sm:text-base"
                    >
                      {sub}
                    </li>
                  ))}
              ...
              <div className="text-gray-500 text-sm sm:text-base">Select a category</div>
            )}
          </div>
        </div>
      </div>
